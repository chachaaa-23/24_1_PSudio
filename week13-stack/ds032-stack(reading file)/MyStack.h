...
class MyStack{
    private:
        ...
        string *list;      //int type의 숫자들을 보관하는 array의 첫번째값. 
        //^ 동적할당을 통해 maxsize만큼의 공간을 만들어줘야 한다. 

    public:
        MyStack(int m);      //최고사이즈 설정해주는 생성자
        ~MyStack();             //destructor
        void initialize();      //스택 초기화
        bool isEmpty() const;   //비어있는 여부확인
        bool isFull() const;    //가득차있는 여부확인
        void push(const string& e);  //reference type 값을 받아와서 stack에 넣는다.
        string* pop();             //top의 값을 반환하고, stack에서 지운다.
